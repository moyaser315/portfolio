...
AI voice assistants, the kind of thing we used to only see in sci-fi movies, are now everywhere in our pockets, in smart homes, and even in cars. Thanks to **Large Language Models (LLMs)**, these assistants grew up fast. They’re no longer those clunky chatbots that barely understood “hello”; now they can actually hold a conversation without embarrassing themselves.  
...
Especially when using an underrepresented language in the model training, like the Arabic Egyptian accent, we will dive into this in a few moments.
The emotion discarding can be tackled using a separate emotion detection model, but that would increase resources consumption and increase cost.

The picture is from [Recent Advances in Speech Language Models: A Survey](https://arxiv.org/html/2410.03751v1)
...
Problem encountered while starting up DiVA is huge resources consumption, tried starting it in kaggle using 2xT4 GPUs, and the kernel restarted because of a buffer overflow before loading it.

---

## Fighting Latency (Because Nobody Likes Waiting)

For a voice assistant to feel natural, it has to be fast, under a few seconds total delay. Nobody wants to ask, “What's the weather?” and get the answer after they’ve already hung the laundry.
...
- **Multilingual ASR:** Whisper is the go-to, supporting loads of languages, and handles the automatic language detection, but doesn't support streaming.
...
Yes, Whisper is great, the go to model out there for multilingual tasks, but let's test it on an actual code switching task using [Egyptian-ASR-MGB-3 ](https://huggingface.co/datasets/MightyStudent/Egyptian-ASR-MGB-3), a 16 hour manually transcribed multi-genre data set collected from different YouTube channels in Egyptian Arabic dialect.
The models tested on it were `Whisper Medium` and `Whisper Large` for transcription and `Seamless M4T TTS` for voice generation.

We used the first 300 audio samples, around 150 minutes of speech, and the metrics used were accuracy and WER(Word Error Rate)
...
As can be seen, unfortunately, both aren't that great but they are the best options currently if you need multi language handling, and as the difference isn't huge in accuracy, choosing the medium model is efficient resource-wise.

For the TTS, Human comparison is a must, but currently I got the generated sound and transcriped it then compared the transcription to the ground truth text and the first transcribed text of Whisper Large, **Why?** I tried to eliminate the error of ASR as much as possible, to get the near accurate result for the TTS metrics
| Text | WER | Acc |
| :--- | :--- | :--- |
| **Ground Truth** | 0.34892 | 0.65107 |
| **Transcription** | 0.57246 | 0.42753 |

The Acc actually got better, becuase the TTS tends to change the dialect to fluent traditional arabic and with no background noise it made the perfect environment for `Whisper` to work better.
So I wanted to hear samples of the TTS outputs to make sure it's good, but it sounded a little robotic and the speech wasn't coherent.

<audio controls>
  <source src="/portfolio/assets/va/sample.wav" type="audio/wav" />
...
- [Recent Advances in Speech Language Models: A Survey](https://arxiv.org/html/2410.03751v1)
- [Robust speech recognition via large-scale weak supervision (Whisper)](http://arxiv.org/abs/2212.04356)
- [Conditional variational autoencoder with adversarial learning for end-to-end text-to-speech (VITS)](http://arxiv.org/abs/2106.06103)
- [Distilling an end-to-end voice assistant without instruction training data (DiVA)](http://arxiv.org/abs/2410.02678)
- [Step-audio 2 technical report](http://arxiv.org/abs/2507.16632)
- [VoiceBench: Benchmarking LLM-based voice assistants](http://arxiv.org/abs/2410.17196)
- [Wav2vec 2.0: A framework for self-supervised learning of speech representations](http://arxiv.org/abs/2006.11477)
- [Toward Low-Latency End-to-End Voice Agents for Telecommunications (Technical Paper)](https://arxiv.org/abs/2508.04721)
- [Creating a voice assistant (Hugging Face Audio Course)](https://huggingface.co/learn/audio-course/chapter7/voice-assistant)
- [Building AI voice agents: Starter guide](https://www.gladia.io/blog/building-ai-voice-agents-starter-guide)
- [How to Build Your Own AI Voice Assistant: Step-by-Step Guide](https://www.uptech.team/blog/how-to-make-an-ai-voice-assistant)
- [The Arabic RAG Leaderboard (Hugging Face Space)](https://huggingface.co/spaces/Navid-AI/The-Arabic-Rag-Leaderboard)


# Trivia

Ever had the hallucination "ترجمة نانسي قنقر"? have a good [laugh](https://github.com/openai/whisper/discussions/2608)